	...
	if (loading) {
		return (
			<Box sx={{ maxWidth: 1200, mx: "auto", px: 2, my: 4 }}>
				<Alert
					severity="info"
					action={<CircularProgress size={18} color="inherit" />}
				>
					Đang tải dữ liệu...
				</Alert>
			</Box>
				...
				<>
					<Grid container spacing={2}>
						{visibleItems.map((item) => {
							...
							const total = Number(item.quantity ?? 0) + Number(item.sold ?? 0);
										...
										>
											...
											<Stack direction="row" spacing={1} alignItems="baseline" flexWrap="wrap" useFlexGap>
												<Typography variant="h6" color="error" fontWeight={800}>
													{Number(item.salePrice ?? 0).toLocaleString("vi-VN")}đ
												</Typography>
											...
											<Typography variant="body2" color="text.secondary">
												Đã bán: {Number(item.sold ?? 0)} / {total}
											</Typography>
